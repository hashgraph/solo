...
export default interface TK8 {
   ...
   * @returns podName IP
   */
  getPodIP(podNameName: string): Promise<any>;
   ...
   */
  testSocketConnection(host: string, port: number): Promise<boolean>;
  ...
  testClusterConnection(context: string, cluster: string): Promise<boolean>;
