...
public record RepositoryAddRequest(Repository repository) implements HelmRequest {
    ...
    public void apply(HelmExecutionBuilder builder) {
        builder.subcommands("repo", "add").positional(repository.name()).positional(repository.url());
    }
}
