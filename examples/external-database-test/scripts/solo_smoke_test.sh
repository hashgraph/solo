...
function start_background_transactions ()
{
  ...
  cd solo
  npm run solo-test -- account create -n external-database-test --create-amount 15 > /dev/null 2>&1 &
  cd -
}
...
function check_monitor_log()
{
  ...
  kubectl get pods -n external-database-test | grep mirror-monitor | awk '{print $1}' | xargs kubectl logs -n external-database-test > mirror-monitor.log
...
{
  kubectl get pods -n external-database-test | grep mirror-importer | awk '{print $1}' | xargs kubectl logs -n external-database-test > mirror-importer.log
...
if [ "$1" == "ACCOUNT_INIT" ]; then
  ...
  npm run solo-test -- account init -n external-database-test
fi
...
helm test mirror -n external-database-test --timeout 10m
check_monitor_log
check_importer_log
