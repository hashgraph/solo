...
COMMANDS=($(task solo -- --help | awk '/Commands:/ {flag=1; next} /Options:/ {flag=0} flag && NF && $1 != "" {print $1}' || true))
...
for cmd in "${COMMANDS[@]}"; do
    ...
    SUBCOMMAND_OUTPUT=$(task solo -- "$cmd" --help 2>/dev/null || true)
...
for cmd in "${COMMANDS[@]}"; do
    echo -e "\n## $cmd" >> "$OUTPUT_FILE"
    echo '```' >> "$OUTPUT_FILE"
    task solo -- "$cmd" --help >> "$OUTPUT_FILE"
    echo '```' >> "$OUTPUT_FILE"
    ...
    for subcmd in "${SUBCOMMANDS[@]}"; do
        ...
        echo '```' >> "$OUTPUT_FILE"
        task solo -- "$cmd" "$subcmd" --help >> "$OUTPUT_FILE"
        echo '```' >> "$OUTPUT_FILE"
    done
done
