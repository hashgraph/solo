...
export function testMirrorNode (namespacePostfix, hederaImage) {
  describe(`MirrorNodeCommand [${namespacePostfix}`, () => {
    const namespace = `mirror-cmd-e2e-${namespacePostfix}`
    ...
    beforeAll(() => {
      bootstrapResp.opts.logger.showUser(`------------------------- START: ${namespace} ----------------------------`)
    })
    ...
    afterAll(async () => {
      ...
      bootstrapResp.opts.logger.showUser(`------------------------- END: ${namespace} ----------------------------`)
    }, 180000)
