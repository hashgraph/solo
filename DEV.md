...
* Before making a commit run `npm run task format`

## E2E tests

* In order to run E2E test, we need to set up cluster and install the chart.
  * Run `./test/e2e/setup-e2e.sh`
  * Run `npm run task test-e2e-standard`, NOTE: this excludes some E2E tests that have their own command
...
* If you like to rerun tests with the same seed, use environment variable `RANDOM_SEED=<integer_number>` with `npm run task test-e2e-standard` command.
  * Example: `RANDOM_SEED=20 npm run task test-e2e-standard`,
